...
- The purple “all simulations” line is the average estimated mean across
  all simulations, which is closer to the true mean `mu`. This makes
  sense, since it takes into account of simulations that rejected and
  failed the rejected the null. The effect size is unbiased compared to
  the underlying population mean. The yellow line with the average
  estimated mean for rejected nulls is overestimated from the true mean,
...
homicide_df \|\> count(victim_age) \|\> arrange(victim_age)

\# to describe some parts of the data

homicide_df \|\> count(victim_sex) \|\> arrange(victim_sex)

homicide_df \|\> count(victim_sex) \|\> arrange(victim_sex)

homicide_df \|\> distinct(city)

homicide_df \|\> ggplot(aes(x = victim_age)) + geom_histogram(binwidth
= 1) + labs(title = “Distribution of Victim Ages”, x = “Age”, y =
“Count”)

# There are 0-102 yo?

homicide_df \|\> count(victim_race) \|\> arrange(victim_race)

homicide_df \|\> count(city_state) \|\> arrange(city_state)

# no tulsa, AL??????

homicide_df \|\> count(disposition) \|\> arrange(disposition)
