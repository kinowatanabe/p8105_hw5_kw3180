...
* The sample average of `mu`across tests for which the null is rejected **isn't** approximately equal to the true value of `mu`. The purple "all simulations" line is the average estimated mean across all simulations, which is closer to the true mean `mu`. This makes sense, since it takes into account of simulations that rejected and failed the rejected the null. The effect size is unbiased compared to the underlying population mean. The yellow line with the average estimated mean for rejected nulls is overestimated from the true mean, particularly when `mu` is small, as the means are among simulations with statistically significant differences. For small `mu`, significant differences between true and estimated mean can occur by chance, so the mean estimate among rejected nulls is inflated.
